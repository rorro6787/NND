...
def compare_model_mask(scans: list) -> dict:
    """
    ...
    for i in range(0, len(image_paths), batch_size):
        ...
        results = segment_image(batch_paths)
...
def tests(image: str = None):
    ...
    pixel_counts, formulas = compare_model_mask(tests)
    print(pixel_counts)
    print(formulas)
...
def get_purple(image: str) -> None:
    ...
    results = segment_image(image_path)[0].masks
...
if __name__ == "__main__":
    ...
    # tests(image="P41_T1_FLAIR_axial_74.png")
    # tests(image=None)
    get_purple(image)
